...
const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbz1KL5EqAz_BBQOLqdzagRBjRbKeTEyFhd4IEKCMS2lM4cFh21waBixt0ZJWkByrzcS7A/exec';
...
contactCards.forEach((card, index)=>{
  ...
  if(strong){
    strong.textContent = '';
    String(data.strong || '').split('\n').forEach((line, lineIndex)=>{
      if(lineIndex) strong.appendChild(document.createElement('br'));
...
function syncContactActionButtons(){
  const baseWidth = window.innerWidth <= 430 ? 128 : window.innerWidth <= 820 ? 138 : window.innerWidth <= 1180 ? 168 : 178;
  contactCards.slice(0, 2).forEach(card=>{
    const action = card.querySelector('a');
    if(!action) return;
    const cardWidth = card.getBoundingClientRect().width || baseWidth;
    const actionMinWidth = window.innerWidth <= 820 ? 112 : 140;
    const actionWidth = Math.max(actionMinWidth, Math.min(baseWidth, cardWidth - 40));
...
const mobileCarousels = [
  {container:'.care-grid', slide:'.care-card', name:'belief', interval:8000},
...
function createMobileCarousel(config){
  ...
  function setActive(nextIndex){
    ...
    container.style.setProperty('--mobile-carousel-index', String(index));
    const centerOffset = Math.floor(slides.length / 2);
    slides.forEach((slide, slideIndex)=>{
      const isActive = slideIndex === index;
      let offset = slideIndex - index;
      if(config.name === 'process' && slides.length > 2){
        offset = ((slideIndex - index + slides.length + centerOffset) % slides.length) - centerOffset;
      }
      slide.style.setProperty('--mobile-slide-offset', String(offset));
      slide.classList.toggle('is-mobile-active', isActive);
      slide.classList.toggle('is-prev', config.name === 'process' && offset === -1);
      slide.classList.toggle('is-next', config.name === 'process' && offset === 1);
      if(config.name === 'process' && mobileCarouselQuery.matches){
        slide.setAttribute('aria-hidden', String(!isActive && Math.abs(offset) !== 1));
      }else{
  ...
  slides.forEach((slide, slideIndex)=>{
    slide.addEventListener('click', ()=>{
      if(!mobileCarouselQuery.matches || config.name !== 'process' || slideIndex === index) return;
  ...
  container.addEventListener('pointerdown', event=>{
    if(!mobileCarouselQuery.matches || (config.name !== 'process' && config.name !== 'belief' && config.name !== 'checklist')) return;
    dragStartX = event.clientX;
  });
  container.addEventListener('pointerup', event=>{
    if(dragStartX === null || !mobileCarouselQuery.matches || (config.name !== 'process' && config.name !== 'belief' && config.name !== 'checklist')) return;
  ...
  function disable(){
    ...
    container.style.removeProperty('--mobile-carousel-index');
    slides.forEach(slide=>{
      slide.classList.remove('mobile-carousel-slide','is-mobile-active','is-prev','is-next');
      slide.setAttribute('aria-hidden', 'false');
      slide.style.removeProperty('--mobile-slide-offset');
...
const estimateTabButtons = [...document.querySelectorAll('[data-estimate-tab]')];
...
function setActiveEstimateTab(type, options={}){
  const {focusInput=false, highlight=false} = options;
  estimateTabButtons.forEach(button=>{
    const isActive = button.getAttribute('data-estimate-tab') === type;
    button.classList.toggle('is-active', isActive);
    button.setAttribute('aria-selected', isActive ? 'true' : 'false');
  });
  estimatePanels.forEach(panel=>{
    const isActive = panel.getAttribute('data-estimate-panel') === type;
  ...
  if(focusInput){
    const targetInput = type === 'townhouse' ? townhouseAreaInput : newHomeAreaInput;
    if(targetInput){
...
estimateTabButtons.forEach(button=>{
  button.addEventListener('click', ()=>{
    const type = button.getAttribute('data-estimate-tab');
    setActiveEstimateTab(type, {focusInput:true});
  });
});
...
document.querySelectorAll('[data-target-estimate]').forEach(card=>{
  ...
  const activate = ()=>{
    ...
    setActiveEstimateTab(targetType, {highlight:true});
    window.requestAnimationFrame(()=>window.scrollTo({top:keepScrollY, left:0, behavior:'auto'}));
  };
